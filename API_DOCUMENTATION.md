...
- **接口名称**：环境数据预测
- **请求路径**：`/api/ai/predict/environment`
- **请求方法**：POST
- **权限要求**：已登录
...
| `predictions[].time` | string | 预测时间 |
| `predictions[].temperature` | number | 预测温度 |
| `predictions[].humidity` | number | 预测湿度 |
| `predictions[].riskLevel` | string | 风险等级 |
...
- **接口名称**：设备故障预测
- **请求路径**：`/api/ai/predict/device-failure`
- **请求方法**：POST
- **权限要求**：已登录
...
- **接口名称**：环境数据分析
- **请求路径**：`/api/ai/analyze/environment`
- **请求方法**：GET
- **权限要求**：已登录
...
| AI环境预测 | POST | `/api/ai/predict/environment` | ✅ 通过 |
