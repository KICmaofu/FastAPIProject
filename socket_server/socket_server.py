...
from app.models.robot import Robot, RobotSensorRecord
...
def get_server_status():
    ...
    return {
        ...
        'port': PORT,
        'uptime': uptime,
        'current_connections': connection_pool['active_connections'],
        'peak_connections': connection_pool['connection_stats']['peak_connections'],
        'total_connections': connection_pool['connection_stats']['total_connections'],
        'failed_connections': connection_pool['connection_stats']['failed_connections'],
        'closed_connections': connection_pool['connection_stats']['closed_connections'],
        'connection_pool_usage': connection_pool['active_connections'] / connection_pool['max_connections'] * 100,
        ...
        'total_data_received_kb': performance_metrics['total_data_received'] / 1024,
        'total_data_sent_kb': performance_metrics['total_data_sent'] / 1024,
        'total_alarms_created': performance_metrics['total_alarms_created'],
        ...
        'config': {
            ...
            'smoke_alarm_threshold': SMOKE_ALARM_THRESHOLD,
            'fire_risk_alarm_threshold': FIRE_RISK_ALARM_THRESHOLD
        }
    }
...
def process_raw_data(raw_data, client_info):
    ...
    try:
        ...
        parsed_data = {
            ...
            'robot_sn': robot_sn,
            'temperature': json_data.get('temperature', 0) or 0,
            'humidity': json_data.get('humidity', 0) or 0,
            'smoke_level': json_data.get('smoke_level') or json_data.get('smokeLevel') or 0,
            'max_single_temp': max_single_temp or 0,
            ...
            'thermal_matrix': thermal_matrix or '',
            'battery': json_data.get('battery', 50) or 50
        }
    ...
    while retry_count < max_retries:
        try:
            ...
            try:
                ...
                sensor_data = RobotSensorRecord(
                    robot_sn=robot_sn,
                    patrol_record_id=None,
                    temperature=parsed_data.get('temperature', 0),
                    humidity=parsed_data.get('humidity', 0),
                    smoke_level=float(parsed_data.get('smoke_level', 0)),
                    max_single_temp=parsed_data.get('max_single_temp', 0) or 0,
                ...
                if should_create_alarm(fire_risk, smoke_level):
                    ...
                    alarm_desc = f"Fire risk level: {fire_risk}, Smoke level: {smoke_level}"
                    if smoke_level > SMOKE_ALARM_THRESHOLD:
                        alarm_desc = f"Smoke alarm detected, level: {smoke_level} PPM"
                    elif fire_risk >= 2:
                        alarm_desc = f"High temperature risk detected, fire risk: {fire_risk}"
                    ...
                    alarm = AlarmInfo(
                        ...
                        area_name=existing_robot.area_name if existing_robot else "Default Area",
                        deal_status=0
                    )
                    
                    db.add(alarm)
                    alarms_created += 1
                    print(f"Alarm created: {alarm_type} - {alarm_level} for robot {robot_sn}")
                
                db.commit()
                print(f"Data saved successfully [ID: {processing_id}]")
                return alarms_created
            except Exception as db_error:
                print(f"Database operation failed [ID: {processing_id}]: {db_error}")
                db.rollback()
                raise db_error
            finally:
                db.close()
            ...
            if retry_count < max_retries:
                print(f"Database save failed [ID: {processing_id}], retrying ({retry_count}/{max_retries}): {error}")
                time.sleep(1)
            else:
                print(f"Database save failed [ID: {processing_id}], max retries reached: {error}")
                traceback.print_exc()
...
def shutdown_server(server=None):
    ...
    if server or server_socket:
        try:
            (server or server_socket).close()
        except:
            pass
